    ...
    <tbody>
        ...
        <c:forEach var="vo2" items="${vo}" varStatus="status">
            <tr>
                ...
                <td style="text-align-last: center;"><h4>${vo2.stdNumber}</h4></td>
                <td>
                    ...
                    <div class="progress progress_sm">
                        ...
                        <c:choose>
                            <c:when test="${credit eq 'A+' || credit eq 'A'}">
                                <c:set var="credit" value="progress-bar progress-bar-striped active" />
                            </c:when>
                            <c:when test="${credit eq 'B+' || credit eq 'B'}">
                                <c:set var="credit" value="progress-bar progress-bar-info progress-bar-striped active" />
                            </c:when>
                            <c:when test="${credit eq 'C+' || credit eq 'C'}">
                                <c:set var="credit" value="progress-bar progress-bar-success progress-bar-striped active" />
                            </c:when>
                            <c:when test="${credit eq 'D+' || credit eq 'D'}">
                                <c:set var="credit" value="progress-bar progress-bar-warning progress-bar-striped active" />
                            </c:when>
                            <c:otherwise>
                                <c:set var="credit" value="progress-bar progress-bar-danger progress-bar-striped active" />
                            </c:otherwise>
                        </c:choose>
						<c:if test="${!empty vo2.jscore}"> 
                        <!--클래스명 가운데  progress-bar-/안넣음  90점 이상 /info 80점 이상 /success 70점 이상 /warning 60점 이상 /danger 50점대 포함 이하-->
                        <div class="${credit}" role="progressbar" data-transitiongoal="${vo2.jscore}"><h2>${vo2.jscore}점</h2></div>
                        </c:if>
                        <c:if test="${empty vo2.jscore}"> 
